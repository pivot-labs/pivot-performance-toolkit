...
final class Lifecycle {
	...
	public static function deactivate(): void {
		...
		if ( file_exists( $config ) ) {
			$config_deleted = wp_delete_file( $config );
	...
	private static function installDropin(): bool {
		...
		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_is_writable -- pre-flight check for the copy() call below, which also uses direct PHP I/O (not WP_Filesystem) so drop-in install works silently on activation without a credentials prompt.
