<x-card-standard
    :title="__('Uninstall Cleanup Policy', 'performance-toolkit')"
    :description="__('Choose whether plugin settings and cache data should be removed when the plugin is deleted from WordPress.', 'performance-toolkit')"
    icon="trash-2"
    tone="indigo"
>
    <form method="post" action="{{ esc_url(admin_url('admin-post.php')) }}" class="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
        ...
        <div class="min-w-0 flex-1 text-sm text-gray-500">
            <label class="flex items-start gap-3">
                <input type="checkbox" name="ptk_remove_data_on_uninstall" value="1" {{ $cleanup_on_uninstall ? 'checked' : '' }} class="mt-1" />
                <span class="text-gray-900">{{ __('Remove all Performance Toolkit data on uninstall', 'performance-toolkit') }}</span>
            </label>
