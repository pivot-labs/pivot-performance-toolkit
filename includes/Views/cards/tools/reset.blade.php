        ...
        <div class="text-sm text-gray-500">
            @if (!empty($last_settings_exported_at_gmt))
                @php
                    ...
                    $formatted = wp_date(__('M j, Y \a\t g:i A', 'pivot-performance-toolkit'), $timestamp);
                @endphp
                {{ sprintf(
                    /* translators: %s: Human-readable date/time of the most recent configuration export. */
                    __('Last configuration export: %s.', 'pivot-performance-toolkit'),
                    $formatted
                ) }}
            @else
                {{ __('No configuration export yet.', 'pivot-performance-toolkit') }}
            @endif
        </div>
        <div class="flex flex-col gap-3 sm:flex-row">
            <form method="post" action="{{ esc_url(admin_url('admin-post.php')) }}">
                <input type="hidden" name="action" value="{{ esc_attr($reset_to_defaults_action) }}" />
                @php wp_nonce_field('pivot_performance_toolkit_reset_to_defaults'); @endphp
                @php submit_button(__('Reset to safe defaults', 'pivot-performance-toolkit'), 'secondary', 'submit', false, array('style' => 'background:#d63638;border-color:#d63638;color:#fff;')); @endphp
            </form>
        </div>
    </div>
</x-card-standard>