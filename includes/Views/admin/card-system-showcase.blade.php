<div class="space-y-8">
    <x-card-standard
        :title="__('Reset Performance Profile', 'performance-toolkit')"
    ...
    >
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="text-sm text-gray-500">
                {{ __('Last configuration export: 2 days ago · Safe defaults profile ready to apply.', 'performance-toolkit') }}
            </div>
            <div class="flex flex-col gap-3 sm:flex-row">
                <button type="button" class="inline-flex min-h-10 items-center justify-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                    {{ __('Export current settings', 'performance-toolkit') }}
                </button>
                <button type="button" class="inline-flex min-h-10 items-center justify-center rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-700">
                    {{ __('Reset to defaults', 'performance-toolkit') }}
                </button>
            </div>
        </div>
    </x-card-standard>
    ...
    <x-card-split
        :title="__('Cache Actions', 'performance-toolkit')"
        :description="__('Run common cache operations without leaving the page. This layout is ideal for utility actions paired with contextual status information.', 'performance-toolkit')"
        :helper="__('Use this pattern for actions like clearing page cache, rebuilding preloads, or updating edge cache providers.', 'performance-toolkit')"
        icon="rocket"
        tone="blue"
    >
        ...
        <x-slot:actions>
            <div class="space-y-3">
                ...
                <button type="button" class="inline-flex w-full min-h-10 items-center justify-center rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
                    {{ __('Clear page cache', 'performance-toolkit') }}
                </button>
                ...
                <button type="button" class="inline-flex w-full min-h-10 items-center justify-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                    {{ __('Preload homepage cache', 'performance-toolkit') }}
                </button>

                <p class="text-xs leading-5 text-gray-500">
                    {{ __('Buttons should expand full-width on mobile and remain visually grouped in the lighter right column on larger screens.', 'performance-toolkit') }}
                </p>
            </div>
        </x-slot:actions>
    </x-card-split>
    ...
    <x-card-sectioned-split
        :title="__('CDN Import / Export Workflow', 'performance-toolkit')"
        :description="__('Use the sectioned split card for multi-step admin workflows where each area needs its own explanation, notice, and set of controls.', 'performance-toolkit')"
    ...
    >
        ...
        >
            <div class="space-y-4">
                ...
                <button type="button" class="inline-flex w-full min-h-10 items-center justify-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                    {{ __('Export settings JSON', 'performance-toolkit') }}
                </button>
            </div>
        </x-card-section>
            ...
            <div class="space-y-4">
                ...
                <div class="flex flex-col gap-3 sm:flex-row">
                    <button type="button" class="inline-flex min-h-10 flex-1 items-center justify-center rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
                        {{ __('Validate import', 'performance-toolkit') }}
                    </button>
                    <button type="button" class="inline-flex min-h-10 flex-1 items-center justify-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                        {{ __('Import settings', 'performance-toolkit') }}
                    </button>
                </div>
            </div>
        </x-card-section>
